...
customCursor({
    ...
    onMove: function (data) {
        if (data.isInViewport) {
            ...
            if (data.isMoving) {
                if (data.isHover) {
                    gsap.to(data.cursor, {opacity: 1, scale: 5, zIndex:9}); 
                } else {
                    gsap.to(data.cursor, {opacity: 1, scale: 1, zIndex: 9999});
                }
            } else {
                if (data.isHover) {
                    gsap.to(data.cursor, {opacity: 1, scale: 5, zIndex:9});
            ...
            // out viewport
            gsap.to(data.cursor, {opacity: 0, scale: 0, zIndex: 9999});
        }
    },
});
...
customCursor({
    targetClass: 'custom-cursor-ball',
    speed: .1,
    hasHover: true,
    onMove: function (data) {
        if (data.isInViewport) {
            ...
            gsap.to(data.cursor, {opacity: 0, zIndex: 9999});
